...
if(!function_exists('plsh_get_rating_stars'))
{
    function plsh_get_rating_stars($display = 'block', $class='')
    {
        if(plsh_is_review())
        {
            ...
            <span itemprop="reviewRating" class="item-rating-hidden"><?php echo round($original_stars/10, 1); ?></span>
            <?php endif; ?>
...
if(!function_exists('plsh_get_banner_by_location'))
{
    function plsh_get_banner_by_location($location='header_ad', $extra_wrapper_class='')
    {
        ...
        if($location && $location['ad_enabled'] == 'on')
        {
            ...
            $adSlug = !empty($location_item['ad_slug']) ? $location_item['ad_slug'] : null;
            $adSize = !empty($location_item['ad_size']) ? $location_item['ad_size'] : null;
            $banner = plsh_get_banner_by_size_and_slug($adSlug, $adSize);
