...
class WC {
    ...
    public static function display_amz_redirect() {
        ...
        add_action('woocommerce_before_single_product', function(){
            ...
            if ( isset( $amz_redirect_info ) && !empty( $amz_redirect_info ) && $amz_redirect_info['amz_redirect_is_active'] == '1' ) {
                add_action( 'custom_woocommerce_variable_add_to_cart_template_not_loaded', function() use ( $amz_redirect_info ) {
                    ?>
                ...
                add_action( 'custom_woocommerce_variable_add_to_cart_template_not_loaded', function() use ( $amz_redirect_info ) {
                    if( !current_user_can('administrator') ) { 
                        echo '<span class="wn_mwh_sold_out">SOLD OUT</span>';
                    }
                } );
            }
        });
    ...
    public static function member_add_meta_box() {
        ...
        foreach ( $screens as $screen ) {
            foreach( self::$metaboxes as $metabox => $data ) {
                $type = $data['type'];
                self::add_custom_meta_box($type, $metabox, $data, $screen);
            }
        }
    }

    static function add_custom_meta_box($type, $metabox, $data, $screen) {
        switch ($type) {
            case 'WYSIWYG_editor': self::wp_editor_metabox($metabox, $data, $screen);
                break;
            case 'amazon_redirect': self::amazon_redirect_metabox_callback($metabox, $data, $screen);
                break;
            default:
                break;
        }
    ...
    static function wp_editor_metabox($metabox, $data, $screen) {
        $title = $data['title'];
    ...
    static function amazon_redirect_metabox_callback( $metabox, $data, $screen ) {
        $title = $data['title'];
    ...
    public static function load_custom_add_to_cart() {
        /**
         * Removing the default wc action of add to the cart for single products 
         * and gettig our custom template.
         */
        remove_action( 'woocommerce_variable_add_to_cart', 'woocommerce_variable_add_to_cart', 30 );
        add_action( 'woocommerce_variable_add_to_cart', array( self::class, 'custom_woocommerce_variable_add_to_cart' ), 30 );

        //Returning Simple price instead of a range
        add_filter( 'woocommerce_variable_price_html', function( $price, $product ){
            $currency_symbol        = get_woocommerce_currency_symbol();
            $product_simple_price   = $product->get_price();

            return $currency_symbol . $product_simple_price;
        }, 10, 2);
    }

    public static function custom_woocommerce_variable_add_to_cart() {
        if( current_user_can('administrator') ) { 
            // add_filter( 'woocommerce_is_purchasable', '__return_false');
            // echo '<input type="hidden" id="wn_product_not_purchasable">'; 
            require_once get_stylesheet_directory() . '/templates/woocommerce/custom_add_to_cart_form_v2.php';
        }
        else {
            do_action( 'custom_woocommerce_variable_add_to_cart_template_not_loaded' );
        }
